...
const App = () => {
  ...
  const getTitleFromChatBot = async (tags : any) => {
    try {
      setTitleLoading(true);

      const titlePrompt = `Generate a catchy title for an item based on these tags only 1 title  and Only title nothing else: ${tags.join(', ')}. The title should be no more than 15 words.`;
      ...
      let generatedTitle = data.candidates?.[0]?.content?.parts?.[0]?.text || "No title available";
      
      generatedTitle = generatedTitle.replace(/[#*]/g, "").split(' ').join(' ');

      setTitle(generatedTitle);

    } catch (error) {
      console.error("Error generating title:", error);
      Alert.alert('Error', 'Failed to generate title.');
    } finally {
  ...
  const getDescriptionFromChatBot = async (tags : any) => {
    try {
      ...
      const descriptionPrompt = `Generate a description for an item based on these tags: ${tags.join(', ')}. The description should be no more than 50 words.`;
      ...
      let generatedDescription = data.candidates?.[0]?.content?.parts?.[0]?.text || "No description available";
      
      generatedDescription = generatedDescription.replace(/[#*]/g, "").split(' ').slice(0, 50).join(' ');
